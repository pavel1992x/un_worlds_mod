...
public class OrironBlock extends WorldsModModElements.ModElement {
	@ObjectHolder("worlds_mod:oriron")
	public static final Block block = null;
	public OrironBlock(WorldsModModElements instance) {
		super(instance, 151);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("oriron", "oriron", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == OrelouhStoneBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 10), Placement.COUNT_RANGE, new CountRangeConfig(2, 32, 32, 81)));
		}
	}
}
