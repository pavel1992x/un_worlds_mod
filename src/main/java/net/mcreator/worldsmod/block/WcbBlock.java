...
public class WcbBlock extends WorldsModModElements.ModElement {
	@ObjectHolder("worlds_mod:wcb")
	public static final Block block = null;
	public WcbBlock(WorldsModModElements instance) {
		super(instance, 72);
	}
	...
	public static class CustomBlock extends FallingBlock {
		public CustomBlock() {
			super(Block.Properties.create(Material.WATER).sound(SoundType.SLIME).hardnessAndResistance(0.95f, 0.5f).lightValue(15));
			setRegistryName("wcb");
		}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("wcb", "wcb", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				if (blockAt.getBlock() == MoongruntBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				if (blockAt.getBlock() == Blocks.AIR.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 15), Placement.COUNT_RANGE, new CountRangeConfig(10, 50, 50, 70)));
		}
	}
}
