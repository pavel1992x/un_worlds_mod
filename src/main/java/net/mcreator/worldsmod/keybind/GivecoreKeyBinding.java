...
package net.mcreator.worldsmod.keybind;
...
public class GivecoreKeyBinding extends WorldsModModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	private long lastpress = 0;
	public GivecoreKeyBinding(WorldsModModElements instance) {
		super(instance, 19);
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					WorldsModMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
